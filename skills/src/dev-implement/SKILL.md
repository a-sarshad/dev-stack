...
دو تلهٔ اصلی: ستون «در طرح دیده می‌شود» را از **screenshot** بنویس نه از
`get_design_context` (LTR-محور)؛ و در یک تصحیح فقط **یک متغیر** عوض کن (ترتیب DOM
یا مقدار logical، نه هر دو). حداقل یک برچسب/دکمهٔ کوتاه در جدول باشد — `w="full"`
باگ جهت را پنهان می‌کند. قاعده بازگشتی است (هر container تودرتو ردیف خودش).

**مکانیزم کامل (دابل-فلیپ، قانون ترجمه، همهٔ استثناها) →
`knowledge/universal/language.md` § «دابل-فلیپ». منبع canonical — اینجا تکرارش نکن.**
