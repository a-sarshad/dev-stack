...
### اختیاری — فقط وقتی حجم توجیهش کنه

| مسیر | نقش |
|------|-----|
| `components/<name>.md` | تله‌ی **یک کامپوننت مشخص** — وقتی `known-bugs.md` اونقدر بزرگ شد که پیدا‌کردن سخت شد |

**قانون تولد فایل:** `components/<x>.md` فقط وقتی ساخته می‌شه که چیزی داشته
باشه که **از هیچ منبع زنده‌ای قابل استنتاج نیست** — باگ تجربه‌شده، تله‌ای که
type-check نمی‌گیره، یا تصمیم composition تکرارشونده.

⛔ **هرگز `variant`/`size`/prop table اینجا ننویس.** این‌ها از سه منبع زنده
همیشه درست در دسترسن: کد خودِ پروژه، داک پرایمیتیو در `node_modules/`، و CLI
خودِ DS. فایل static فقط drift می‌کنه — و چون agent بهش اعتماد می‌کنه و verify
نمی‌کنه، **بدتر از نبودشه**.

وقتی این پوشه ساخته شد، `known-bugs.md` فقط cross-component می‌مونه (CLI،
preset/config، RTL کلی، رجیستری، tooling) و بالاش یه جدول مسیریابی می‌گیره.

نمونه‌ی پیاده‌شده: `shadcn-ui/components/` (۶ فایل) — قرارداد کاملش در
