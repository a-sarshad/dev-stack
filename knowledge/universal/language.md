...
در locale فارسی (و بقیهٔ RTLها) هر عددی که **کمیت** است ارقام بومی نشان می‌دهد
(`۰۱۲۳۴۵۶۷۸۹`)؛ `en` و LTRها لاتین می‌مانند.

**کمیت در برابر شناسه:**

| نوع | مثال | در فارسی |
|---|---|---|
| کمیت | تعداد، مبلغ، درصد، صفحه، امتیاز، ساعت | ارقام فارسی |
| شناسه | شمارهٔ پرواز، کد گیت، پلاک، شمارهٔ سفارش، کد تخفیف، SKU، id | **لاتین می‌ماند** |

شناسه ماهیت عددی ندارد — یک برچسب است که تصادفاً رقم دارد. مرتب کردن، جمع زدن،
مقایسهٔ بزرگ‌تر/کوچک‌تر رویش بی‌معنی است. اگر شک داری: «۲ تا از اینها» جمع می‌بندد؟
بله → کمیت. نه → شناسه.

**این یک قانون لایهٔ نمایش است، نه data.** مقدار در state/سرویس/محاسبه لاتینِ تمیز
می‌ماند؛ فقط render فارسی می‌شود. مقداری که فرم به بک‌اند می‌فرستد هم لاتین است —
وگرنه DB سرِ ارقام فارسی می‌شکند.

**دو مسیر:**
- عدد داخل جمله (interpolate در تابع ترجمه `t()`) → خروجی fa را از تابع
  digit-substitution رد کن. **فقط جایگزینی رقم، بدون جداکنندهٔ هزارگان** — تا سال
  «۱۴۰۵» به «۱٬۴۰۵» نشکند.
- عددِ خام کمیت در markup → یک `formatNumber(n, locale)` که جداکنندهٔ locale
  (`٬` برای fa، `,` برای en) + ارقام درست می‌دهد (`Intl.NumberFormat` بومی این را
  می‌دهد).

**ورودی کاربر:** با هر کیبورد (فارسی/عربی/لاتین) تایپ می‌شود → قبل از محاسبه یا
ارسال با یک `toLatinDigits()` نرمالایز کن (فارسی U+06Fx و عربی-هندی U+066x → لاتین).

پیاده‌سازی مرجع: Vitrina (`src/utils/numbers.ts`) و kish-airport (`src/lib/numbers.ts`).
enforce: `dev-engine` rule `persian-numerals` (نبودِ فارسی‌سازی در JSXِ کمیت را flag می‌کند).
